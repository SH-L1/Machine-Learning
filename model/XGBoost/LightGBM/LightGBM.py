    ...
    for param_name in param_names:
        ...
        if other_params:
            ...
            for name, group in grouped:
                label = ", ".join(
                    [f"{p}={v}" for p, v in zip(other_params, name if isinstance(name, tuple) else (name,))])
                group.sort_values(by=param_name).plot(x=param_name, y='mean_r2_score', ax=plt.gca(), label=label,
                                                      marker='o', linestyle='-')
        else:
            results_df.sort_values(by=param_name).plot(x=param_name, y='mean_r2_score', ax=plt.gca(), marker='o',
                                                       linestyle='-')
...
def train_final_model():
        ...
        for d in param_grid['max_depth']:
            for lr in param_grid['learning_rate']:
                ...
                for train_index, val_index in kf.split(X_train):
                    ...
                    y_cv_train, y_cv_val = y_train[primary_target].iloc[train_index], y_train[primary_target].iloc[
                        val_index]
    ...
    tuned_lgbm = lgb.LGBMRegressor(
        objective='regression',
        random_state=42, n_jobs=-1, verbosity=-1, **best_params
    )
    tuned_multioutput_model = MultiOutputRegressor(tuned_lgbm)
